...
public class Multiply {
    ...
    private static class Job implements Runnable{
        ...
        public Job(String url, MessageDigest messageDigest, MysqlConnectionPoolDataSource dataSource) {
    ...
    public static void main(String[] args) throws IOException, NoSuchAlgorithmException, InterruptedException {
        ...
        CountDownLatch countDownLatch = new CountDownLatch(detailsUrlList.size());
        for(String urls:detailsUrlList){

            pool.execute(new Job(url,dataSource));
        }
        while (successCount.get()+failureCount.get()!=detailsUrlList.size()){
            System.out.println("一共 "+detailsUrlList.size()+"首诗，成功"+
                    successCount.get()+"首"+"，失败"+failureCount.get();
